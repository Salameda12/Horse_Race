...
def calculate_payout(winning_horse):
    ...
    # 정산금 라벨 업데이트
    payout_text = f"정산금 (해당 말의 배율 {dividend:.2f} / 해당 말의 배팅을 한 사람의 수 = 최종 배율 {payout_per_person:.2f}):\n"
    for bettor in bets[winning_horse]:
        payout_text += f"{bettor}: {bet_amount * payout_per_person:.2f}원\n"
...
winning_horse_label.grid(row=6, column=0, padx=10, pady=5)
winning_horse_entry = ttk.Entry(root)
winning_horse_entry.grid(row=6, column=1, padx=10, pady=5)

# 승리한 말 설정 버튼
set_winning_horse_button = ttk.Button(root, text="승리한 말 설정", command=set_winning_horse)
set_winning_horse_button.grid(row=7, column=0, columnspan=2, padx=10, pady=5)

# 정산금 표시 라벨
payout_var = tk.StringVar()
payout_var.set("정산금:")
payout_label = ttk.Label(root, textvariable=payout_var)
payout_label.grid(row=8, column=0, columnspan=2, padx=10, pady=5)
...
rules_label.grid(row=11, column=0, columnspan=2, padx=10, pady=5)
